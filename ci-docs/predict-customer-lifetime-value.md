...
- إنشاء مقاطع عملاء استراتيجية استنادًا إلى قيمتها المحتملة لتشغيل حملات مخصصة ذات جهود مستهدفة تتعلق بالمبيعات والتسويق والدعم.
...
قبل البدء، عليك إظهار ما تعنيه مدة بقاء العميل (CLV) لأعمالك. في الوقت الحالي، ندعم خدمات التنبؤ بقيمة عمر العميل (CLV) القائمة على الحركات. تعتمد قيمة العميل التي تم التنبؤ بها على محفوظات حركات الأعمال. لإنشاء التنبؤ، ستحتاج إلى أذونات [مساهم](permissions.md) واحد على الأقل.
...
- معرف العميل: المعرف الفريد لمطابقة الحركات مع عميل فردي

- محفوظات الحركات: سجل محفوظات الحركات مع مخطط البيانات الدلالية أدناه
    - **معرف الحركة**: معرف فريد لكل حركة
    - **تاريخ الحركة:** التاريخ، ويفضل أن يكون طابعًا زمنيًا لكل حركة
    - **مبلغ الحركة:** القيمة المالية (على سبيل المثال، الإيراد أو هامش الربح) لكل حركة
    - **تسمية معينة للمرتجعات (اختياري):** قيمة منطقية تعني ما إذا كانت الحركة عبارة عن مرتجعات 
    - **معرفة المنتج (اختياري):** هوية منتج المشارك في الحركة
...
- خصائص البيانات المقترحة:
    - بيانات تاريخية كافية: عام واحد على الأقل من بيانات الحركات. ويفضل وجود عامين أو ثلاثة من بيانات الحركات للتنبؤ بقيمة عمر العميل (CLV)‬ لمدة عام واحد.
    - عمليات الشراء المتعددة لكل عميل: في العادة، اثنان أو ثلاث حركات على أقل لكل معرف عميل، ويفضل أن يتم ذلك عبر تواريخ متعددة.
...
> [!NOTE]
> - يتطلب النموذج سجل الحركات لعملائك. يمكن حاليًا تكوين كيان واحد فقط لسجل الحركات. في حالة وجود كيانات شراء/بيع متعددة، يمكنك توحيدها في Power Query قبل بدء عملية استيعاب البيانات.
...
1. حدد معنى **العملاء النشطاء** لعملك. قم بتعيين الإطار الزمني حيث يجب أن يكون للعميل حركة واحدة على الأقل تعتبر نشطة. سيتنبأ النموذج قيمة مدى بقاء العميل للعملاء النشطاء فقط. 
...
1. قم بتعريف القيمة المئوية **للعميل ذي القيمة العالية** لتمكين النموذج من تقديم نتائج تتناسب مع تعريف أعمالك.
    - **حساب النموذج (مستحسن)**: يقوم النموذج بتحليل بياناتك ويحدد ما قد يكون عليه العميل ذي القيمة العالية لأعمالك استنادًا إلى محفوظات حركات العميل. يستخدم النموذج قاعدة استكشافية (من وحي قاعدة 80/20 أو مبدأ pareto) للعثور على نسبة العملاء من ذوي القيمة العالية تعتبر النسبة المئوية للعملاء الذين ساهموا في نسبة 80% من الإيرادات التراكمية لأعمالك في الفترة السابقة عملاء من ذوي القيمة العالية. يساهم عادةً أقل من 30-40% من العملاء في 80% من الإيرادات المتراكمة. ومع ذلك، قد يختلف هذا الرقم وفقًا للأعمال والصناعة.    
...
1. في خطوة **البيانات المطلوبة**، حدد **إضافة بيانات** لخيار **محفوظات حركات العميل**، واختر الكيان الذي يوفر معلومات محفوظات الحركات/الشراء كما هو موضح في [المتطلبات الأساسية](#prerequisites).
...
1. في خطوة **بيانات إضافية (اختياري)**، حدد **إضافة بيانات** ضمن **دعم رؤى النموذج باستخدام بيانات نشاط إضافية**. اختر كيان نشاط العميل الذي يوفر معلومات نشاط العميل كما هو موضح في [المتطلبات الأساسية](#prerequisites).
...
1. قم بإضافة بيانات اختيارية للعملاء أو حدد **التالي**.
...
1. في خطوة **بيانات إضافية (اختياري)**، حدد **إضافة بيانات** ضمن **دعم رؤى النموذج بشكل أكبر باستخدام بيانات العميل الإضافية**.

1. بالنسبة إلى **الكيان**، اختر **العميل: CustomerInsights** لتحديد جدول ملف تعريف العميل الموحد الذي يتم تحديده لبيانات سمة العميل. بالنسبة إلى **معرف العميل**، اختر **System.Customer.CustomerId**.
...
1. حدد **حفظ** بعد تعيين السمات التي يجب أن يستخدمها النموذج للمساعدة في التنبؤ بقيمة عمر العميل.
...
- **قيمه العملاء حسب القيمة المئوية‬**: باستخدام تعريفك للعملاء من ذوي القيمة العالية، يتم تجميع العملاء في قيمة منخفضة وقيمة عالية، استنادًا إلى تنبؤات قيمة مدة بقاء العميل، كما يظهر في المخطط. ومن خلال تمرير الماوس فوق الأشرطة في بالمشاهدة فوق الأشرطة في المُدرج التكراري، يمكنك رؤية عدد العملاء في كل مجموعة ومتوسط قيمة مدة بقاء العميل في هذه المجموعة. قد تساعدك هذه البيانات إذا كنت ترغب في [إنشاء مقاطع عملاء](segments.md) استنادًا إلى توقعات قيمة مدة بقاء العميل.
